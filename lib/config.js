...
export const config = {
  ...
  probe: {
    ...
    skipSubstrings: json?.probe?.skip_models_with_substring ?? ['image', 'midjourney', 'suno'],
    tier: (process.env.PROBE_TIER || json?.probe?.tier || 'free').toLowerCase(),
  },

  rateLimit: {
    multPerMin: int(process.env.RATE_LIMIT_MULT_PER_MIN, json?.rate_limit?.mult_per_min ?? 10),
  },
...
export function summarize() {
  return {
    ...
    probeTier: config.probe.tier,
    rateLimitMultPerMin: config.rateLimit.multPerMin,
  };
}
