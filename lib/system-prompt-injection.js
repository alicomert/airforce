...
// DEFAULT KAPALI. Tecrube: contract enjekte etmek zayif modelleri tool
// formatini karistirmaya ittirebiliyor (ornek: Anthropic native tool_use yerine
// OpenAI JSON {"name":"Read"} basiyorlar, Claude Code'da "No such tool" hatasi
// ureptiyor). Ayrica zorla tool cagirmak model bilinmeyen konularda sikismasina
// ve bos cevap donmesine sebep oluyor. Istemedigin surece kapali kalsin.
// Acmak istersen: INJECT_SYSTEM_PROMPT=1
export const SYSTEM_PROMPT_INJECTION_ENABLED = process.env.INJECT_SYSTEM_PROMPT === '1';
...
export function buildToolContract(requestBody) {
  ...
  // Not: metin MINIMAL ve YUMUSAK tutuldu. Rigid "MUST" kurallari modeli
  // bilmedigi konularda sikistirip bos cevap doneduruyor. Bunun yerine
  // oneri tarzi ifadeler kullanilir. Model istedigi zaman text cevap verebilir,
  // soru sorabilir, aciklama yapabilir. Aksiyon gerektiren durumlarda tool
  // cagirmayi tercih etmesi istenir, dayatma yok.
  const lines = [
    MARKER_START,
    'Tool usage guidance (from airforce-compat-proxy):',
    '',
    'You have access to these tools for interacting with the user\u2019s environment:',
    ...signatures,
    '',
    'When the user asks you to perform an action (create/modify a file, run a command, explore the project, search content, fetch a URL, etc.), prefer calling the relevant tool over describing the action in plain text. You may still answer in plain text when the user is asking a question, wants an explanation, or when you genuinely need clarification.',
    '',
    'Guidance:',
    '- Use relative paths unless the user gave you an absolute one.',
    '- Match each tool\u2019s required schema fields exactly.',
    '- If you are unsure about something (missing info, ambiguous request, unknown domain), it is OK to ask a short clarifying question instead of guessing or calling an unrelated tool.',
    MARKER_END
  ];
