...
// Client'in zaten SUBSTANTIAL bir system prompt'u gonderdigini tespit et.
// OpenCode, Claude Code, Cursor gibi istemciler kendi detayli tool guidance
// system prompt'u gonderirler. Bizim contract bunu eklemek karmaya yol
// acabilir: guclu modeller 2 paralel talimat setini reconcile etmeye calisirken
// empty response verebiliyorlar. Yapisal kural: eger mevcut system prompt
// >= 500 karakter ise, model zaten yeterli guidance aliyor, contract atla.
function clientAlreadyHasSubstantialSystem(requestBody) {
  // Anthropic format: requestBody.system (string or array)
  if (typeof requestBody?.system === 'string') {
    return requestBody.system.length >= 500;
  }
  if (Array.isArray(requestBody?.system)) {
    const total = requestBody.system.reduce((sum, b) => {
  ...
  for (const msg of messages) {
    ...
    if (typeof msg.content === 'string' && msg.content.length >= 500) return true;
    if (Array.isArray(msg.content)) {
      const total = msg.content.reduce((sum, b) => {
        return sum + (typeof b?.text === 'string' ? b.text.length : 0);
      }, 0);
...
// dondurur (enjekte edilecek bir sey yok). Ayrica client zaten substantial
// bir system prompt gonderdi ise atla (yukaridaki detaylari icerir).
export function buildToolContract(requestBody) {
  ...
  // Client zaten detayli guidance veriyorsa contract ekleme - double-prompting
  // ister istemez modelin kafasini karistirir.
  if (clientAlreadyHasSubstantialSystem(requestBody)) return null;
