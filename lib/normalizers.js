...
const TOOL_NAME_ALIASES = {
  bash: ['bash', 'runcommand', 'command', 'shell', 'terminal', 'exec'],
  read: ['read', 'readfile', 'readfilecontents', 'read_file', 'read_file_contents', 'openfile'],
...
function coerceToolInput(toolName, input, schema) {
  ...
  if (normalizedToolName === 'read') {
    result.filePath = normalizeEscapedPath(result.filePath ?? result.value);
    result.file_path = normalizeEscapedPath(result.file_path ?? result.filePath ?? result.value);
...
function canonicalizeToolCalls(toolCalls, requestBody) {
  ...
  return toolCalls.map((toolCall) => {
    ...
    return {
      ...toolCall,
      name: resolvedName,
      input: coerceToolInput(resolvedName, toolCall.input, schema)
    };
  });
}
...
function parseHeader(header) {
  ...
  if (xmlNameMatch || xmlServerNameMatch) {
    ...
    const argsMatch = trimmed.match(XML_ARGUMENTS_RE);
    const parsedArgs = argsMatch ? parseJsonSafe(argsMatch[1].trim()) : null;
...
export function extractPseudoToolCalls(text) {
  ...
  if (toolCalls.length === 0) {
    ...
    if (looseToolCalls.length > 0) {
      return {
        text: residualLines.join('\n').trim(),
        toolCalls: looseToolCalls
      };
    }

    const fencedMatches = [...cleaned.matchAll(FENCED_JSON_RE)];
    ...
    const bareJsonToolCall = normalizeJsonToolCandidate(cleaned);
    if (bareJsonToolCall) {
      return {
        text: '',
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  const normalizedToolCalls = canonicalizeToolCalls(dropEmptyBrokenToolCalls(combineFragmentedToolCalls(extractedToolCalls)), requestBody);
