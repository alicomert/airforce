...
// Tool input'larinda path/filename/pattern/source/destination gibi string
// field'lara upstream modelin sizdirdigi XML artifact'lari temizle.
//   "index.html</Read-Ray>" -> "index.html"
//   "foo.md<command_message>..." -> "foo.md"
//   "bar.txt(fetching file)" -> "bar.txt"
// Bash command'lara gore DAHA AZ agresif: komut semantigine gerek yok, sadece
// sona yapismis HTML/XML benzeri markup + trailing parantezli etiket temizlenir.
function sanitizeToolInputString(value) {
  if (typeof value !== 'string') {
    return value;
  }
  let out = value;
  // Instrumentation block'lari (opening + content + closing)
  out = out.replace(INSTRUMENTATION_BLOCK_RE, '');
  // Herhangi bir XML-benzeri opening/closing tag (path icinde olmamali)
  out = out.replace(/<\/?[A-Za-z_][\w-]*(?:\s[^<>]*)?\/?>/g, '');
  // Trailing UI label (ornek: "file.html(fetching file listing)")
  let prev;
  do {
    prev = out;
...
function normalizeEscapedPath(value) {
  ...
  // Once XML/UI artifact temizligi, sonra Windows backslash escape fix
  const sanitized = sanitizeToolInputString(value);
  return /^[A-Za-z]:\\\\/.test(sanitized) ? sanitized.replace(/\\\\/g, '\\') : sanitized;
}
...
// Upstream model ajanlari (Claude Code, OpenCode vb.) user mesajlarina
// bazi instrumentation XML tag'lari ekliyor ve zayif modeller bunlari komutun
// icine kopyaliyor. Ornek hatali komutlar:
//   "find . -type f | head -80\n<command_message>find . -type f | head -80"
//   "ls -la\n<system-reminder>Your operational mode has changed...</system-reminder>"
// Bunlar bash tarafinda syntax hatasi veya yanlis komut yorumu yaratir.
// Tanidik instrumentation tag'lari icerik + closing + acilma dahil SILINIR.
// NOT: tag isimleri hardcode edilmiyor - regex genericist. <command*>, <system*>,
// <local-command*> gibi bilinen instrumentation prefix'leri yakalar.
// Hard-coded tag isimleri DEGIL, Claude Code / OpenCode / ChatGPT vb. istemci
// frameworklerinin kullandigi yaygin prefix'leri yakalayan bir pattern.
// Yeni framework prefix'leri cikarsa pattern'e eklenir (agents/instrumentation).
const INSTRUMENTATION_TAG_RE = /<\/?(?:command[-_][\w-]+|system[-_][\w-]+|system-reminder|local[-_]command[-_][\w-]+|Read[-\w]*|Write[-\w]*|Edit[-\w]*|Glob[-\w]*|Grep[-\w]*|tool[-_]result|tool[-_]use|tool[-_]call)(?:\s[^<>]*)?\/?>/gi;
// Full XML-like tag (acilma + icerik + kapanma) - multi-line.
const INSTRUMENTATION_BLOCK_RE = /<(command[-_][\w-]+|system[-_][\w-]+|system-reminder|local[-_]command[-_][\w-]+|tool[-_]result|tool[-_]use|tool[-_]call)(?:\s[^<>]*)?>[\s\S]*?<\/\1>/gi;
...
function sanitizeCommand(value) {
  ...
  let out = stripCommandUiWrapping(value)
    ...
    // Claude Code / OpenCode instrumentation tag'leri (block + standalone)
  ...
  // Leading tire fix: bash komutu tek tire ile baslamaz (cogu durumda).
  // Upstream model bazen '-find', '-ls' gibi hatali baslangic uretiyor.
  // Sadece '-[a-z]+' pattern'i + bosluk/satir sonu: tireyi at.
  // Exit-early cases: '-' tek basina (stdin), '--' flag, '-c' flag. Bunlara dokunma.
  //   -find . -type  -> find . -type
  //   -ls -la        -> ls -la
  //   -c 'cmd'       -> -c 'cmd' (dokunulmaz - 2 karakter, flag olabilir)
  //   --help         -> --help (dokunulmaz)
  //   - input        -> - input (dokunulmaz, stdin marker)
  out = out.replace(/^-([a-z]{3,})(\s|$)/i, '$1$2');
...
function coerceToolInput(toolName, input, schema) {
  ...
  if (normalizedToolName === 'read' || normalizedToolName === 'edit') {
    ...
    // Ortak kaynak: filePath / file_path / path / file / filename / value herhangi
    // birinde yol olabilir. Tum alanlari birbirinden dolduruyoruz ki istemci hangi
    // sema adini kullaniyorsa onu gorsun (OpenCode 'filePath' istiyor, Claude
    // Code 'file_path' istiyor).
    const pathSource = result.filePath ?? result.file_path ?? result.path ?? result.value ?? pathFromCmd ?? result.file ?? result.filename;
    result.filePath = normalizeEscapedPath(pathSource);
    result.file_path = normalizeEscapedPath(pathSource);
    result.path = normalizeEscapedPath(pathSource);
  ...
  // Path/pattern-type string alanlarindan XML/UI artifact'lari temizle.
  // content/text/body ve command alanlarina DOKUNMA - dosya icerigi ve komut
  // semantik olarak string artifact'i barindirabilir (HTML yaz\u0131l\u0131yor olabilir).
  // NOT: Beyaz liste / kara liste stratejisi - hangi alanlar sanitize edilir
  // listesi burada aciktir ve yeni tool alanlari gerekirse eklenir.
  const PATH_LIKE_FIELDS = ['file_path', 'filePath', 'path', 'pattern', 'file', 'filename', 'source', 'destination', 'old_string', 'oldString', 'new_string', 'newString', 'search', 'find', 'replace', 'replacement'];
  for (const fieldName of PATH_LIKE_FIELDS) {
    if (typeof result[fieldName] === 'string') {
      result[fieldName] = sanitizeToolInputString(result[fieldName]);
    }
  }
  ...
  if (normalizedToolName === 'write' || normalizedToolName === 'edit') {
    // Write/Edit icin path alanlari her iki schema (OpenCode filePath, Claude Code
    // file_path) uyumlulugu icin tum alanlar ayni kaynaktan doldurulur.
    const pathSource = result.filePath ?? result.file_path ?? result.path ?? result.file ?? result.filename ?? (looksLikePath(result.value) ? result.value : undefined);
    result.filePath = normalizeEscapedPath(pathSource);
    result.file_path = normalizeEscapedPath(pathSource);
    result.path = normalizeEscapedPath(pathSource);
  }
  ...
  if (normalizedToolName === 'write' && result.content === undefined) {
    if (result.value && !looksLikePath(result.value)) {
  ...
  if (normalizedToolName === 'write') {
    // Path alanlarinin sync'i yukarida yapildi. Burada yalnizca content fallback.
    if (result.content === undefined || result.content === '') {
      result.content = result.text ?? result.body ?? result.value ?? '';
    }
  }
...
export function applyOpenAiChatNormalization(payload, requestBody) {
  ...
  const choices = payload.choices.map((choice) => {
    ...
    // CASE A: upstream zaten structured tool_calls dondurmus.
    // Buradaki sorun: model 'arguments' JSON'unu Anthropic schema ile uretebilir
    // ('file_path') ama istemci (OpenCode) OpenAI schema bekliyor ('filePath').
    // Zod validator failsiz ('Invalid input: expected string, received undefined').
    // Cozum: her tool_call'in arguments'ini parse et, canonicalizeToolCalls ile
    // path field'larini istemcinin schema'sina gore doldur, tekrar stringify et.
    // canonicalizeToolCalls istemciden gelen 'tools' listesinin schema'sina gore
    // filePath vs file_path vs path field adlarini hepsini dolduruyor - OpenCode
    // schema'sinda hangisi required ise onu bulur.
    if (Array.isArray(message.tool_calls) && message.tool_calls.length > 0) {
      const rawExtracted = message.tool_calls.map((tc) => {
        let input = {};
        const argsJson = tc?.function?.arguments;
        if (typeof argsJson === 'string') {
          try {
            input = argsJson.trim() ? JSON.parse(argsJson) : {};
          } catch {
            input = {};
          }
      ...
      const canonicalized = canonicalizeToolCalls(rawExtracted, requestBody);
      const collapsed = COLLAPSE_PARALLEL_TOOL_CALLS
        ? collapseParallelToolCalls(canonicalized)
        : canonicalized;
      return {
        ...choice,
        ...
        message: {
          ...message,
          tool_calls: collapsed.map((toolCall, index) => ({
            id: toolCall.id && String(toolCall.id).startsWith('call_') ? toolCall.id : `call_${String(toolCall.id ?? '').replace(/^toolu_/, '') || crypto.randomUUID().replace(/-/g, '')}`,
            type: 'function',
            index,
            function: {
              name: toolCall.name,
    ...
    // CASE B: sadece text content var, tool_calls yok. Text icinde pseudo tool
    // emisyonu olabilir; cikar + sentezle + normalize et.
