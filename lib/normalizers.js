...
function getReadTargetsFromAssistantMessage(message) {
  ...
  // Anthropic format: content array icinde tool_use block'lari
  if (Array.isArray(message.content)) {
    for (const block of message.content) {
      if (block?.type !== 'tool_use') continue;
      if (normalizeName(block?.name) !== 'read') continue;
      const input = block?.input || {};
      const target = normalizeEscapedPath(input.file_path ?? input.filePath ?? input.path ?? input.file ?? input.filename);
      if (target) targets.push(target);
    }
  }

  // OpenAI Chat format: content: null|string + tool_calls array
  // Bu fix eklenmeden once sadece Anthropic format tanindigi icin OpenAI
  // path'inde "ayni dosyayi tekrar tekrar Read et" loop'u olusabiliyordu:
  // suppressRepeatedReadToolCalls gecmis Read'leri gormuyordu.
  if (Array.isArray(message.tool_calls)) {
    for (const tc of message.tool_calls) {
      const name = tc?.function?.name ?? tc?.name ?? '';
      if (normalizeName(name) !== 'read') continue;
...
export function applyOpenAiChatNormalization(payload, requestBody) {
  ...
  const choices = payload.choices.map((choice) => {
    ...
    if (Array.isArray(message.tool_calls)) {
      ...
      // IMPORTANT: content must be non-empty otherwise isEffectivelyEmpty()
      // returns true and triggers empty-retry loop. Keep original content if
      // it has substance, otherwise add a minimal notice.
      if (emittedToolCalls.length === 0) {
        const preservedContent = typeof message.content === 'string' && message.content.trim()
          ? message.content
          : 'The model requested invalid tool calls. Continuing without tool use.';
        return {
          ...choice,
          finish_reason: 'stop',
          message: {
