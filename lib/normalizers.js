...
// Kullaniciya gonderilecek "model cevabi bozuk, yeniden dene" mesaji.
// Dile bagimsiz (teknik Ingilizce), ama client UI'i dil'e cevirebilir.
const INCOMPLETE_COMPLETION_MESSAGE = 'The upstream model returned a malformed completion (claimed to finish but did not produce the required tool call). Please retry or switch to a stronger model.';
...
// Model cevabinda "bozuk stray/unclosed XML-like artifact" var mi?
// Model-agnostik yapisal kontrol: kapanis tag'i var ama ayni tag'in acilisi
// yoksa VE tag adi standart HTML/XML elementi degilse -> bozuk cevap.
//
// Bu yaklasim GPT-3.5'tan Llama'ya, DeepSeek'ten Gemini'ye HER model icin
// calisir. Model'in "iç monolog" tag'i </think> olabilir, </reasoning>,
// </scratchpad>, </planning>, </rationale>... vs. Hicbirini hardcode
// etmiyoruz - sadece "bilinen HTML elementi DEGIL + acilisi yok" kontrolu.
function hasMalformedFinalText(content, payload, requestBody) {
  if (payload?.stop_reason === 'tool_use') return false;
  const textBlocks = Array.isArray(content)
    ? content.filter((b) => b?.type === 'text' && typeof b?.text === 'string' && b.text.trim())
    : [];
  if (textBlocks.length === 0) return false;

  const combinedText = textBlocks.map((b) => b.text).join('\n');

  // Tum kapanis tag'lerini tara (hepsini, hardcoded liste YOK)
  const closeTagRe = /<\/([a-zA-Z][\w:-]*)\s*>/g;
  let match;
  let foundStrayTag = false;
  while ((match = closeTagRe.exec(combinedText)) !== null) {
    const tagName = match[1];
    // Legitimate HTML/XML ise atla
    if (isLegitimateCloseTag(tagName)) continue;
    // Ayni tag'in acilisi var mi?
    const openRe = new RegExp(`<${tagName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
    if (openRe.test(combinedText)) continue; // Cift tam, legit
    // Stray kapanis tag'i - model "jenerator frame"ini bozdu
    foundStrayTag = true;
    break;
  }

  if (!foundStrayTag) return false;

  // Session'da prior tool_use var mi? (En az 2)
  // Bu kontrol false positive'i azaltiyor: ilk turda stray tag olsa da
  // kullanici muhtemelen sadece text cevap bekliyordu. Ama prior history'de
  // tool zaten calisiyorsa, bu gercekten "proje ortasinda model sapti" durumu.
  const messages = Array.isArray(requestBody?.messages) ? requestBody.messages : [];
  ...
  if (priorToolUseCount < 2) return false;
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  if (normalizedToolCalls.length === 0) {
    ...
    // tekrar basiyor. Bu Claude Code / OpenCode arayuzunde Write yaptiktan
    // SONRA ham markdown kod gibi gozukuyor ve kullanici "yazmadi" saniyor.
    // Dil-bagimsiz: sadece content match'ine bakar.
    stripPostWriteDuplicateFencedBlocks(content, requestBody);

    // "Model tamamlamis iddia ediyor ama aslinda yapmamis" tespiti:
    //   - end_turn (model bitirdi)
    //   - Hic tool_use yok
    //   - Text'te stray/unclosed XML-like artifact (</think>, </answer>, vs)
    //     Bu model cevabinin bozuk olduguna isaret eden yapisal bir sinyal.
    //   - Session'da en azindan 1 prior keşif tool_use var (yani model is
    //     yapiyormus gibi goruyor ama bitirisinde tool_use uretmedi)
    // Bu durumda kullaniciya net bir mesaj don ki stall olmasin, kazara
    // dosya yazma riski olmasin.
    if (hasMalformedFinalText(content, payload, requestBody)) {
      return {
        ...payload,
        content: [{
          type: 'text',
          text: INCOMPLETE_COMPLETION_MESSAGE
        }],
