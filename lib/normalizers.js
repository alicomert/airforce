...
// <thinking>...</thinking> - Anthropic-style chain-of-thought tag. Temizle.
// Bu UI'da asistant'in ic konusmasini gostermemeli.
...
// tek-kelime tool isimleri (context, message, parameters) geliyorsa bu gercek
// bir shell command degildir, upstream model tool adini komuta yanlislikla
// ustune yazmistir. Bu tarz bash call'larini dusur.
function isBogusBashCommand(command) {
  if (typeof command !== 'string') return false;
  const trimmed = command.trim();
  if (!trimmed) return true;
  // Tek kelime ve tanidik tool adi
  if (/^(?:bash|read|write|edit|glob|grep|task|agent|skill|context|message|parameters?|command|tool_use|tool_call)$/i.test(trimmed)) {
    return true;
  }
  // Salt `:message=...` veya `-message=...` veya `--message=...` gibi orphan flag'lar
  // (gercek komut yok, bunlar model'in kendi kendine fisildamasi).
...
function cleanText(value) {
  // Remove closed think/thinking tags and unclosed variants
  let out = value
