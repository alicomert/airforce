...
function coerceToolInput(toolName, input, schema) {
  ...
  if (normalizedToolName === 'read' || normalizedToolName === 'edit') {
    ...
    result.filePath = normalizeEscapedPath(result.filePath ?? result.value ?? pathFromCmd ?? result.path ?? result.file ?? result.filename);
...
function hasInitCommandContext(requestBody) {
  if (!Array.isArray(requestBody?.messages)) {
    return false;
  }
  return requestBody.messages.slice(-3).some((message) => {
    const content = flattenMessageText(message?.content);
    return /<command-message>\s*\/init\s*<\/command-message>|(?:^|\s)\/init(?:\s|$)/im.test(content);
  });
}
...
const INIT_COMMAND_RE = /^\/init\s*$/im;

function extractInitCommand(text) {
  const source = String(text ?? '').trim();
  if (INIT_COMMAND_RE.test(source)) {
    return { name: 'bash', input: { command: 'find . -maxdepth 2 -type f | head -80', description: 'Inspect repository files' } };
  }
  return null;
}

function synthesizeInitExploration(text, requestBody) {
  const compactText = String(text ?? '').trim();
  if (!compactText || !hasInitCommandContext(requestBody)) {
    return [];
  }

  const initCommand = extractInitCommand(compactText);
  if (initCommand) {
    const registry = buildToolRegistry(requestBody);
    const bashEntry = [...registry.entries()].find(([, meta]) => meta.normalized === 'bash');
    const globEntry = [...registry.entries()].find(([, meta]) => meta.normalized === 'glob');
    const toolCalls = [];
    if (bashEntry) {
      toolCalls.push({
        id: `toolu_${crypto.randomUUID().replace(/-/g, '')}`,
        name: bashEntry[0],
        input: { command: 'find . -maxdepth 2 -type f | head -80', description: 'Inspect repository files' }
      });
    }
    if (globEntry) {
      toolCalls.push({
  ...
  const looksExploratory = /\b(i'll|i will|let me|start|begin|explor|analyz|understand|architecture|codebase|repo|repository|key files|working with)\b/i.test(compactText);
  if (!looksExploratory) {
    return [];
  }
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  if (normalizedToolCalls.length === 0) {
    const textContent = content
      ...
      .join('\n\n')
      .trim();
    const initToolCalls = synthesizeInitExploration(textContent, requestBody);
    const followupToolCalls = initToolCalls.length > 0 ? initToolCalls : synthesizeExplorationFollowup(textContent, requestBody);
    ...
    // kilitlenmesi yaratir. stop_reason'u end_turn'e indir ve bos text
    // biraktiysak minik bir ack text koy ki client bir sonraki turu baslatsin.
    if (content.length === 0) {
