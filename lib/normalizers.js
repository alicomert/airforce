...
function coerceToolInput(toolName, input, schema) {
  ...
  if ((normalizedToolName === 'write' || normalizedToolName === 'edit') && result.filePath === undefined) {
    result.filePath = normalizeEscapedPath(result.path ?? result.file ?? result.filename);
  }

  if (normalizedToolName === 'write' || normalizedToolName === 'edit') {
    result.file_path = normalizeEscapedPath(result.file_path ?? result.filePath ?? result.path ?? result.file ?? result.filename);
  }
...
function cleanText(value) {
  ...
  return value.replace(THINK_TAG_RE, '').replace(/<think>[\s\S]*?(?=\n|$)/gi, '').replace(/\r\n/g, '\n').trim();
}
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  if (normalizedToolCalls.length === 0) {
    ...
    const recentToolResultContext = Array.isArray(requestBody?.messages) && requestBody.messages
      .slice(-2)
      .some((message) => message?.role === 'user' && Array.isArray(message?.content) && message.content.some((block) => block?.type === 'tool_result'));
    if (recentToolResultContext) {
      const followupToolCalls = synthesizeExplorationToolCalls('follow up after tool result', {
        ...requestBody,
        messages: [{
          role: 'user',
