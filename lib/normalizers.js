...
function coerceToolInput(toolName, input, schema) {
  ...
  if (normalizedToolName === 'write') {
    result.filePath = normalizeEscapedPath(result.filePath ?? result.file_path);
    ...
    if (typeof result.content !== 'string' && typeof result.body === 'string') {
...
function dropEmptyBrokenToolCalls(toolCalls) {
  return toolCalls.filter((toolCall) => {
    ...
    if (normalized === 'delete' && !input.path && !input.file_path && !input.filePath && !input.value) {
      return false;
    }
    return true;
  });
...
function canonicalizeToolCalls(toolCalls, requestBody) {
  ...
  return toolCalls.map((toolCall) => {
    ...
    if (
      ...
      const originalNormalizedName = normalizeName(toolCall.name);
      if (
        input.value === undefined &&
        input.arguments === undefined &&
        SHELL_COMMAND_ALIASES.has(originalNormalizedName)
      ) {
        input.command = toolCall.name;
      } else if (TOOL_NAME_ALIASES.delete.includes(originalNormalizedName)) {
        const deletePath = normalizeEscapedPath(input.path ?? input.file_path ?? input.filePath ?? input.value);
        if (deletePath) {
          input.command = `rm -f -- ${shellQuote(deletePath)}`;
          input.description = input.description ?? `Delete file ${deletePath}`;
        }
      }
    }
    return {
...
function hasRecentAssistantReadToolUse(requestBody) {
  return Array.isArray(requestBody?.messages) && requestBody.messages
    ...
    .some((message) =>
      ...
      message.content.some((block) => block?.type === 'tool_use' && normalizeName(block?.name) === 'read')
    );
}
...
function synthesizeRemainingFilesFollowup(text, requestBody) {
  ...
  if (!/remaining files|key files|more closely|for completeness/i.test(compactText)) {
    return [];
  }
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  if (normalizedToolCalls.length === 0) {
    const textContent = content
      ...
      .join('\n\n')
      .trim();
    const followupToolCalls = synthesizeRemainingFilesFollowup(textContent, requestBody);
