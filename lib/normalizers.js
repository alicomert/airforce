...
function dropEmptyBrokenToolCalls(toolCalls) {
  return toolCalls.filter((toolCall) => {
    ...
    if ((normalized === 'bash' || normalized === 'read' || normalized === 'glob' || normalized === 'grep') && Object.keys(input).length === 0) {
      return false;
    }
...
function shouldSynthesizeExplorationToolCalls(text, requestBody) {
  ...
  const hasPriorAssistantToolUse = messages.some((message) =>
    message?.role === 'assistant' &&
    Array.isArray(message?.content) &&
    message.content.some((block) => block?.type === 'tool_use')
  );
  if (hasPriorAssistantToolUse) {
    return false;
  }
  ...
  return (
    recentUserText.includes('<command-message>') ||
    recentUserText.includes('<ide_opened_file>') ||
    /@[^\s]+\.[a-z0-9]+/i.test(recentUserText) ||
    /[a-z]:\\|\/[a-z0-9._-]+/i.test(recentUserText)
  );
}

function synthesizeExplorationToolCalls(text, requestBody) {
  if (!shouldSynthesizeExplorationToolCalls(text, requestBody)) {
    return [];
  }
    ...
    if (meta.normalized === 'bash') {
      synthetic.push({
        ...
        input: {
          ...
          description: 'Inspect top-level repository files and directories'
        }
      });
      continue;
    }
...
function hasRecentToolResultContext(requestBody) {
  return Array.isArray(requestBody?.messages) && requestBody.messages
    .slice(-2)
    .some((message) => message?.role === 'user' && Array.isArray(message?.content) && message.content.some((block) => block?.type === 'tool_result'));
}

function shouldSynthesizeFollowupToolCalls(text, requestBody) {
  if (!hasRecentToolResultContext(requestBody)) {
    return false;
  }

  const compactText = String(text ?? '').trim();
  if (!compactText || compactText.length > 220) {
    return false;
  }
  ...
  return /^(let me|i('| a)?ll|i will)\b/i.test(compactText);
}

function synthesizeFollowupToolCalls(text, requestBody) {
  if (!shouldSynthesizeFollowupToolCalls(text, requestBody)) {
    return [];
  }
  ...
  for (const [toolName, meta] of registry.entries()) {
    if (meta.normalized === 'read') {
      for (const filePath of ['CLAUDE.md', 'AGENTS.md', 'README.md']) {
        synthetic.push({
          id: `toolu_${crypto.randomUUID().replace(/-/g, '')}`,
          name: toolName,
    ...
    if (meta.normalized === 'bash') {
      synthetic.push({
        ...
        name: toolName,
        input: {
          command: 'find . -maxdepth 2 -type f | sort | head -120',
          description: 'Inspect key repository source files'
        }
      });
    }
  }
...
function parseHeader(header) {
  ...
  const name = rawName.replace(ARG_KEY_RE, '').trim();
...
export function applyAnthropicNormalization(payload, requestBody) {
  ...
  if (normalizedToolCalls.length === 0) {
    const textContent = content
      ...
      .join('\n\n')
      .trim();
    const syntheticToolCalls = synthesizeExplorationToolCalls(textContent, requestBody);
    ...
    const followupToolCalls = !textContent
      ? synthesizeExplorationToolCalls('follow up after tool result', {
          ...requestBody,
          messages: [{
            role: 'user',
            content: '<command-message>continue tool workflow</command-message>'
          }]
        })
      : synthesizeFollowupToolCalls(textContent, requestBody);
