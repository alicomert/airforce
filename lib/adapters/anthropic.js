...
import { resolveModel } from '../capability.js';
...
export async function handleAnthropicMessages(req, res, body) {
  ...
  let payload;
  try {
    const { json } = await upstreamJson('POST', '/v1/messages', upstreamBody);
    payload = json;
  } catch (err) {
    log.error('anthropic upstream error', { err: err.message, status: err.status });
