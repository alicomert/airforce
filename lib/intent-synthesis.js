...
// Assistant mesajindaki tool_use bloklarini dondur. Iki formatu da destekler:
//
//   Anthropic: { role: 'assistant', content: [{ type: 'tool_use', name, input, id }, ...] }
//   OpenAI Chat: { role: 'assistant', content: null|string, tool_calls: [{ id, type: 'function', function: { name, arguments: '<json>' } }] }
//
// Bu fonksiyon eklenmeden once intent synthesis sadece Anthropic formatini
// taniyordu; OpenAI Chat path'inde geçmis tool_use'lari "gormedigi icin"
// priorCategories hep bos doniyor, her turda Glob fallback sentezleyip loop
// olusturuyordu ("✱ Glob **/* tekrar tekrar..." bugu).
function getAssistantToolUses(message) {
  if (!message || message.role !== 'assistant') return [];
  const result = [];
  if (Array.isArray(message.content)) {
    for (const block of message.content) {
      if (block?.type === 'tool_use' && typeof block?.name === 'string') {
        result.push({ name: block.name, input: block.input ?? {}, id: block.id });
      }
    }
  }
  if (Array.isArray(message.tool_calls)) {
    for (const tc of message.tool_calls) {
      const name = typeof tc?.function?.name === 'string'
        ? tc.function.name
        : (typeof tc?.name === 'string' ? tc.name : null);
      if (!name) continue;
      ...
      result.push({ name, input, id: tc?.id });
    }
  }
  return result;
}

// Hem Anthropic hem OpenAI Chat format'indaki assistant mesajinin "content"
// tarafini dondur. Text, reasoning, ve tool_use farklari icin normalized.
function hasAssistantToolUses(message) {
  return getAssistantToolUses(message).length > 0;
}
...
// Bir mesajdan tool_result text'ini cikar. Iki format:
//   Anthropic: { role: 'user', content: [{ type: 'tool_result', content: ... }] }
//   OpenAI Chat: { role: 'tool', content: '<string>' } veya tool_call_id'li
function extractToolResultText(message) {
  if (!message) return '';
  if (message.role === 'tool' && (typeof message.content === 'string' || Array.isArray(message.content))) {
    return flattenBlockText(message.content);
  }
...
function getLastToolResultContext(requestBody) {
  const messages = Array.isArray(requestBody?.messages) ? requestBody.messages : [];
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    const toolResultText = extractToolResultText(messages[i]);
    if (!toolResultText) continue;
...
function getAllAssistantMessages(requestBody) {
  const messages = Array.isArray(requestBody?.messages) ? requestBody.messages : [];
  // Hem Anthropic (content: array) hem OpenAI Chat (content: string|null,
  // tool_calls: array) mesajlarini say. Eski versiyon sadece content array'i
  // olanlari saydigi icin OpenAI path'inde history "gorunmuyordu" -> Glob
  // fallback'i her turda tetikleniyor ve loop oluyordu.
  return messages.filter((msg) =>
    msg?.role === 'assistant' &&
    (Array.isArray(msg?.content) || typeof msg?.content === 'string' || Array.isArray(msg?.tool_calls))
  );
}
...
function categorizeToolName(name) {
  ...
  for (const [category, meta] of Object.entries(TOOL_CATEGORIES)) {
    if (meta.nameAliases.includes(normalized)) return category;
  }
  return null;
}
...
function getPreviousToolCategories(requestBody) {
  ...
  for (const msg of assistants) {
    for (const toolUse of getAssistantToolUses(msg)) {
      const category = categorizeToolName(toolUse.name);
...
function getLastAssistantToolCategories(requestBody) {
  ...
  for (const toolUse of getAssistantToolUses(last)) {
    const category = categorizeToolName(toolUse.name);
    if (category) categories.add(category);
  }
  return categories;
...
function assistantReadTargets(message) {
  if (!message) return [];
  const result = [];
  for (const toolUse of getAssistantToolUses(message)) {
    if (normalizeName(toolUse.name) !== 'read') continue;
    const input = toolUse.input || {};
    const target = input.file_path ?? input.filePath ?? input.path ?? input.file ?? input.filename;
...
function findRecentExplorationToolResult(requestBody) {
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    for (const toolUse of getAssistantToolUses(previousAssistant)) {
      const cat = categorizeToolName(toolUse.name);
      if (cat) categoriesInPrev.add(cat);
    }
    ...
    if (!cameFromExplorationTool) continue;
    if (extractAllFilenamesFromText(toolResultText).length === 0) continue;
    return { toolResultText, previousAssistant };
  }
  return null;
}
...
function trySynthesizeList(requestBody, toolRegistry, upstreamStopReason) {
  ...
  // Cross-turn loop guard: onceki herhangi bir turda (bu proxy sentezledigi
  // veya istemcinin kendi cagirdigi) bir Glob/list tool'u zaten cagrilmissa,
  // tekrar sentezlemek sonucu degistirmeyecek, sadece loop olusturur.
  // (OpenAI Chat path'inde assistant turlari dogru okunmazsa bu her turda
  // tetikleniyordu; getAssistantToolUses fix'i ile birlikte bu ekstra guard
  // "Glob **/* tekrar tekrar" patolojisini imkansiz kilar.)
