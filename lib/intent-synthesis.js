...
//
// Dil-agnostik kalabilmek icin exploratory fallback cok kisitli:
// sadece DETERMINISTIK sinyallerle calisir:
//
//   (A) Kullanicinin son mesajinda bir SLASH-COMMAND ('/init', '/explore',
//       '/analyze', '/overview') var - bunlar Claude Code, OpenCode gibi
//       istemcilerin standart komutlari, dil-bagimsizdir.
//
//   Veya:
//   (B) Kullanicinin mesajinda hic dosya adi yok VE hic prior tool_use yok VE
//       upstream modeli `stop_reason: 'tool_use'` ile donduruyor ama hicbir
//       tool_use ya da text vermiyor (yani net olarak model bir tool
//       cagirmaya CALISTI ama basaramadi).
//
// Bunlar disi durumlarda sentez yapilmaz; "hello" / "merhaba" / chitchat
// sorulari icin Glob sentezlemek agresif olur.

// Standart slash-command pattern'leri (dil-bagimsiz, istemci tarafindan
// standartlastirilmis). Claude Code / OpenCode / Cursor gibi istemcilerin
// hepsinde ayni.
// Claude Code `/init` komutunu genelde `<command-name>/init</command-name>`
// icinde gonderir, o yuzden regex BASLANGIC sinirinda whitespace bekleme -
// XML tag sonundan veya string basindan da yakala. Sag sinir hala word-
// boundary (\b) cunku `/initialize` olmasin.
const EXPLORE_SLASH_COMMAND_RE = /(?:^|[\s>"'`(\[{])\/(?:init|explore|analyze|analyse|overview|understand|review|inspect|walkthrough)\b/i;
...
function trySynthesizeList(requestBody, toolRegistry, upstreamStopReason) {
  ...
  // Sinyal (A): slash-command
  const hasSlashCommand = EXPLORE_SLASH_COMMAND_RE.test(userText);

  // Sinyal (B): upstream bir tool cagirmaya calisti ama veremedi
  const upstreamTriedToolButFailed = upstreamStopReason === 'tool_use';

  if (!hasSlashCommand && !upstreamTriedToolButFailed) return [];
