...
- Two source files do all the work:
  ...
  - `lib/normalizers.js` — all the weird tool-call / `<think>` / XML-ish text normalization (~1600 lines; this is the bulk of the logic)
...
- **No project-specific fallback synthesis.** Earlier versions of the proxy would inject Linux-only commands (`find . -maxdepth 2`, `/workspace/...`, `/init`) when upstream returned no tool call. That broke cross-platform and project-agnostic usage and has been removed entirely. Do NOT reintroduce "helpful" synthesis — the proxy should only clean up what upstream produced, never make up commands on its own. If an agent wants exploration, it must come from the model.
