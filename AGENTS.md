...
- `sanitizeCommand` strips several classes of upstream garbage from bash commands: (1) trailing XML/markdown tag artifacts, (2) trailing UI-render labels like `(fetching file listing)` / `(running command)` via `UI_LABEL_SUFFIX_RE`, (3) Claude Code / OpenCode instrumentation tags like `<command_message>`, `<system-reminder>`, `<local-command-stdout>` that weak models leak into the command (`INSTRUMENTATION_TAG_RE`, `INSTRUMENTATION_BLOCK_RE`), (4) erroneous leading single-dash prefix (`-find .` → `find .`) that upstream models sometimes produce. Preserve all of these; every one traces back to a real broken log.
...
- **Structured OpenAI `tool_calls` are canonicalized on response.** Previously `applyOpenAiChatNormalization` would short-circuit when upstream already emitted a `tool_calls` array and pass it through untouched. That meant tool argument JSON went to the client exactly as the upstream model produced it — including wrong schema shape. Now each `tool_calls[].function.arguments` is JSON-parsed, run through `canonicalizeToolCalls` (same path as Anthropic `tool_use` blocks), and re-stringified. This is what makes the proxy actually OpenCode-compatible for weak models.
